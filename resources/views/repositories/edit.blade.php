    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <form action="{{ route('repositories.edit', $repository) }}" method="POST", class="max-w-mg">
                    @csrf
                    @method('PUT')

                    <label for="block font-medium text-sm text-gray-700">Url</label>
                    <input type="text" name="url" id="url" class="mt-1 form-input block w-full" value="{{ $repository->url }}">
